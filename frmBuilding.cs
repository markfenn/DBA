    ...
    public partial class frmBuilding : Form
    {
        ...
        private void grdTenants_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            ...
            else
            {
                try
                {
                    int cellSpaceId = (int)grdTenants.Rows[e.RowIndex].Cells[2].Value;

                    if (cellSpaceId == BillingSpaceId)
                    {
                        DataGridViewCell cell = (grdTenants.Rows[e.RowIndex].Cells[1] as DataGridViewLinkCell);
                        cell.Style.BackColor = Color.Wheat;

                        cell = (grdTenants.Rows[e.RowIndex].Cells[0] as DataGridViewLinkCell);
                        cell.Style.BackColor = Color.Wheat;
                    }
                }
                catch { }
